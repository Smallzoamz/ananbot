...
export default function GuildDashboard() {
    ...
    const handleDeleteSelective = async () => {
        ...
        try {
            ...
            const data = await res.json();
            if (data.status) {
                alert("Selective deletion started! โจ");
    ...
    const handleAction = async (action, template = "Shop") => {
        ...
        try {
            ...
            if (action === "setup") {
                if (template === "Shop") {
                    finalExtra = { options: setupFlavor === "Full" ? ["Nitro", "Stream Status", "เน€เธกเนเธ” Boost"] : ["Nitro", "Stream Status"] };
            ...
            const data = await res.json();
            if (data.status) {
                alert(`Successfully triggered: ${action}! โจ`);
            } else {
    ...
    const handleClaimReward = async (missionKey) => {
        ...
        try {
            ...
            const data = await res.json();
            if (data.success) {
                alert(`Claimed ${data.xp_added} XP! ๐ธโจ`);
                fetchMissions();
            } else {
                alert(`Error: ${data.error}`);
            }
    ...
    if (loading) return <div className="loader">๐ธ Fetching Server Secrets...</div>;
    if (!guildData) return <div className="loader">โ Server Not Found</div>;
        ...
        <div className="mee6-container">
            ...
            <aside className="mee6-sidebar">
                <div className="guild-switcher-wrapper" ref={guildRef}>
                    <div className={`guild-selector glass ${isDropdownOpen ? 'active' : ''}`} onClick={() => setIsDropdownOpen(!isDropdownOpen)}>
                        <div className="guild-current">
                            ...
                            <div className="chevron">{isDropdownOpen ? "โ" : "โ"}</div>
                        </div>
                    </div>
                    {isDropdownOpen && (
                ...
                <div className="sidebar-footer">
                    <button className="invite-btn" onClick={() => router.push("/servers?redirect=false")}>โ {t.sidebar.switch}</button>
                </div>
            </aside>
                ...
                <header className="mee6-header">
                    ...
                    <div className="header-right">
                        {userPlan.plan_type === "premium" ? (
                            <div className="plan-badge-top premium animate-glow">PREMIUM โจ</div>
                        ) : userPlan.plan_type === "pro" ? (
                            <div className="plan-badge-top pro">PRO ๐’</div>
                        ) : (
                            <button className="premium-btn" onClick={() => setShowPricingModal(true)}>Upgrade to Premium ๐‘‘</button>
                        )}
                        ...
                        <div className="modal-overlay blur-in" onClick={() => setShowPricingModal(false)}>
                            <div className="pricing-modal glass animate-pop" onClick={e => e.stopPropagation()}>
                                <div className="pricing-header">
                                    <h2>Choose Your <span>Power</span> ๐‘‘</h2>
                                    <p>Unlock exclusive features to grow your community ๐ธ</p>
                                    <button className="close-x" onClick={() => setShowPricingModal(false)}>ร—</button>
                                </div>
                                <div className="pricing-grid">
                                    <div className="pricing-card">
                                        ...
                                        <ul className="p-features">
                                            <li>โ… Basic Template Deploy</li>
                                            <li>โ… Daily Missions</li>
                                            <li>โ… General Commands</li>
                                        </ul>
                                        <button className="p-btn disabled">Current Plan</button>
                                    </div>
                                    <div className="pricing-card featured pro">
                                        <div className="p-tier">PRO ๐’</div>
                                        <div className="p-price">$4.99 <span>/month</span></div>
                                        <ul className="p-features">
                                            <li>โ… All Free Features</li>
                                            <li>โ… Pro Badge on Profile</li>
                                            <li>โ… anan-terminal Access</li>
                                            <li>โ… Advanced Management</li>
                                        </ul>
                                        <button className="p-btn pro">Get Pro ๐€</button>
                                    </div>
                                    <div className="pricing-card featured premium">
                                        <div className="p-badge-promo">BEST VALUE</div>
                                        <div className="p-tier">PREMIUM โจ</div>
                                        <div className="p-price">$9.99 <span>/month</span></div>
                                        <ul className="p-features">
                                            <li>โ… Everything in Pro</li>
                                            <li>โ… Lifetime Updates</li>
                                            <li>โ… Custom Bot Branding</li>
                                            <li>โ… Priority Support 24/7</li>
                                        </ul>
                                        <button className="p-btn premium">Get Premium ๐‘‘</button>
                                    </div>
                                </div>
                                <div className="pricing-footer">
                                    <p>An An will handle everything for Papa! ๐ธ๐’–</p>
                                </div>
                            </div>
                        </div>
                    )}
                    {showConfirmModal && (
                        <div className="modal-overlay blur-in">
                            <div className="modal-card mini-card glass animate-pop">
                                <h3>โ ๏ธ {t.dashboard.modal.confirm}</h3>
                    ...
                    <div className="hero-banner mission-center">
                        ...
                        <div className="hero-text">
                            ...
                            <div className="hero-buttons">
                                <button className="hero-btn primary" onClick={() => router.push(`/servers/${guildId}/leaderboard`)}>Do a Mission ๐€</button>
                        ...
                        <div className="mission-card-wrapper animate-float">
                            <div className="mission-leaderboard glass animate-pop">
                                <span>๐ Leaderboard</span>
                            </div>
                            <div className="mission-card glass">
                                <div className="mc-header">
                                    <div className="mc-title">
                                        <span className="mc-icon">๐ฏ</span>
                                                ...
                                                <div className="mi-info">
                                                    ...
                                                    {m.current_count < m.target_count ? (
                                                        ...
                                                        <div className="mi-claimed-status">
                                                            {m.is_claimed ? <span>Claimed โ…</span> : <button className="claim-btn-mini" onClick={() => handleClaimReward(m.key)}>Claim Reward! ๐</button>}
                                                        </div>
                                                    )}
                                                </div>
                                                {m.current_count >= m.target_count && m.is_claimed && <div className="mi-check">โ“</div>}
                                            </div>
                                        ))
                                    ) : (
                            ...
                            {(() => {
                                const plugins = [
                                    ...
                                    },
                                    {
                                        ...
                                        badgeClass: "p-badge",
                                        category: "Essentials",
                                        desc: "Deploy professional server templates (Shop, Community, etc.) with An An ๐ธ",
                    ...
                    <div className="modal-overlay blur-in" style={{ zIndex: 4000 }}>
                        <div className="modal-card perms-modal glass animate-pop">
                            <div className="modal-header">
                                ...
                                <button className="modal-close" onClick={() => setShowPermsModal(false)}>ร—</button>
                            </div>
                        ...
                        <div className={`modal-card glass animate-pop-slow ${setupStep === 'custom_roles' || setupStep === 'custom_zones' ? 'setup-modal' : ''}`}>
                                    ...
                                    <p>
                                        {setupStep === 'custom_roles' ? t.dashboard.modal.stepCustomRoles :
                                            setupStep === 'custom_zones' ? t.dashboard.modal.stepCustomZones :
                                                `Phase ${setupStep} of 3 โ€ข ${setupStep === 1 ? t.dashboard.modal.step1 : setupStep === 2 ? t.dashboard.modal.step2 : t.dashboard.modal.step3}`}
                                    </p>
                                </div>
                                <button className="modal-close" onClick={() => setShowSetupModal(false)}>ร—</button>
                            </div>

                            {setupStep === 1 && (
                                <div className="setup-grid">
                                    <div className="setup-option" onClick={() => { setSelectedTemplate("Shop"); setSetupStep(2); }}>
                                        <div className="so-icon">๐’</div>
                                        <div className="so-info">
                                            <h4>Shop (เธฃเนเธฒเธเธเนเธฒ)</h4>
                                            <p>Optimized for selling nitro & services.</p>
                                        </div>
                                        <div className="so-arrow">โ’</div>
                                    </div>

                                    <div className="setup-option" onClick={() => { setSelectedTemplate("Community"); setSetupStep(2); }}>
                                        <div className="so-icon">๐ฎ</div>
                                        <div className="so-info">
                                            <h4>Community (เธเธญเธกเธกเธน)</h4>
                                    ...
                                    <div className="setup-option" onClick={() => { setSelectedTemplate("Fanclub"); setSetupStep(3); }}>
                                        <div className="so-icon">๐‘‘</div>
                                        <div className="so-info">
                                            <h4>Fanclub (เนเธเธเธเธฅเธฑเธ)</h4>
                            ...
                            {setupStep === 2 && (
                                <div className="setup-grid animate-fade">
                                    {selectedTemplate === "Shop" ? (
                                        <>
                                            <div className="setup-option" onClick={() => { setSetupFlavor("Full"); handleAction("setup", "Shop"); }}>
                                                <div className="so-icon">๐’</div>
                                                <div className="so-info">
                                                    <h4>Full Pack (เธเธฑเธ”เน€เธ•เนเธก)</h4>
                                            ...
                                            <div className="setup-option" onClick={() => { setSetupFlavor("Standard"); handleAction("setup", "Shop"); }}>
                                                <div className="so-icon">๐’</div>
                                                <div className="so-info">
                                                    <h4>Standard Pack (เธกเธฒเธ•เธฃเธเธฒเธ)</h4>
                                        ...
                                        <>
                                            <div className="setup-option" onClick={() => { setSetupFlavor("Friend"); handleAction("setup", "Community"); }}>
                                                <div className="so-icon">๐‘ฅ</div>
                                                <div className="so-info">
                                                    <h4>เธชเธฒเธขเนเธเธ—เธ—เธฑเนเธงเนเธ (Friend)</h4>
                                            ...
                                            <div className="setup-option" onClick={() => { setSetupFlavor("Game"); setSetupStep(3); }}>
                                                <div className="so-icon">๐ฎ</div>
                                                <div className="so-info">
                                                    <h4>เธชเธฒเธขเน€เธเธกเน€เธกเธญเธฃเน (Game)</h4>
                                    ...
                                    <button className="modal-btn ghost" onClick={() => setSetupStep(1)}>โ Back to selection</button>
                                </div>
                            )}
                                ...
                                <div className="setup-config-area animate-fade">
                                    <div className="config-group">
                                        ...
                                        <input
                                            ...
                                            autoFocus
                                        />
                                        <span className="input-hint">An An will create dedicated zones for each item! ๐ช</span>
                                    </div>
                            ...
                            {setupStep === 'custom_roles' && (
                                <div className="setup-config-area animate-fade">
                                    <div className="custom-list">
                                        {customRoles.map((role, idx) => (
                                            <div key={idx} className="custom-item glass">
                                                ...
                                                <button className="del-btn" onClick={() => setCustomRoles(customRoles.filter((_, i) => i !== idx))}>ร—</button>
                                            </div>
                                        ))}
                                    ...
                                    <div className="modal-actions-v" style={{ marginTop: '20px' }}>
                                        <button className="modal-btn primary" onClick={() => setSetupStep('custom_zones')}>Next Step โ’</button>
                                        <button className="modal-btn ghost" onClick={() => setSetupStep(1)}>โ Back</button>
                                    </div>
                                </div>
                            )}
                            ...
                            {setupStep === 'custom_zones' && (
                                <div className="setup-config-area animate-fade">
                                            ...
                                            {customZones.map((zone, idx) => (
                                                <div key={idx} className={`zone-tab ${activeZoneIndex === idx ? 'active' : ''}`} onClick={() => setActiveZoneIndex(idx)}>
                                                    <div className="zone-tab-main">
                                                        ...
                                                        />
                                                        <button className="zone-del" onClick={(e) => { e.stopPropagation(); setCustomZones(customZones.filter((_, i) => i !== idx)); }}>ร—</button>
                                                    </div>
                                                ...
                                                {customZones[activeZoneIndex]?.channels.map((ch, idx) => (
                                                    <div key={idx} className="chan-row">
                                                        <div className="chan-item-main">
                                                            <span className="ch-type-icon">{ch.type === 'text' ? '#' : '๐”'}</span>
                                                            <input
                                                                value={ch.name}
                                                                onChange={(e) => {
                                                                    const NewZ = [...customZones];
                                    ...
                                    <div className="role-legend glass animate-fade" style={{ marginTop: '20px', padding: '12px 20px', borderRadius: '15px' }}>
                                        <div className="legend-title">๐’ก Role Legend (Tips)</div>
                                        <div className="legend-items">
                                            ...
                                            <div className="legend-note">Click the dots to toggle access for each category/channel! โจ</div>
                                        </div>
                                    </div>

                                    <div className="modal-actions-v" style={{ marginTop: '30px' }}>
                                        <button className="modal-btn primary" onClick={() => handleAction("setup", "Custom")}>Create My Server! โจ</button>
                                        <button className="modal-btn ghost" onClick={() => setSetupStep('custom_roles')}>โ Back to Roles</button>
                                    </div>
                                </div>
                            )}
                ...
                {showSelectiveModal && (
                    <div className="modal-overlay blur-in">
                        <div className="modal-card wide-card glass animate-pop" style={{ textAlign: 'left' }}>
                            <button className="modal-close" onClick={() => setShowSelectiveModal(false)}>ร—</button>

                            <div className="selective-header">
                                <div className="m-icon">๐—๏ธ</div>
                                <div className="m-title">
                                    <h3>Structure Manager</h3>
                                    <p>เธเธฑเธ”เธเธฒเธฃเนเธเธฃเธเธชเธฃเนเธฒเธเน€เธเธดเธฃเนเธเน€เธงเธญเธฃเนเนเธเธเน€เธเธฒเธฐเธเธ</p>
                                </div>
                            </div>
                                ...
                                {serverStructure.map(cat => (
                                    <div key={cat.id} className="discord-cat-section">
                                        <div className="discord-cat-header" onClick={() => toggleSelection(cat.id, "category", cat.channels)}>
                                            ...
                                            <span className="cat-arrow">{cat.name === "UNCATEGORIZED" ? "๐“" : "โต"}</span>
                                            ...
                                            {cat.channels.map(ch => (
                                                <div key={ch.id} className={`discord-ch-item ${selectedIds.includes(ch.id) ? 'selected' : ''}`} onClick={(e) => { e.stopPropagation(); toggleSelection(ch.id); }}>
                                                    ...
                                                    <span className="ch-icon">
                                                        {ch.type === "0" || ch.type === "text" ? "#" : (ch.type === "2" || ch.type === "voice" ? "๐”" : "๐’ฌ")}
                                                    </span>
                            ...
                            <div className="modal-footer">
                                ...
                                <div className="modal-actions-row">
                                    <button className="modal-btn ghost" onClick={() => setShowSelectiveModal(false)}>เธขเธเน€เธฅเธดเธ</button>
                                    <button
                                        className="modal-btn danger"
                                        onClick={handleDeleteSelective}
                                        disabled={selectedIds.length === 0 || isDeleting}
                                    >
                                        {isDeleting ? "เธเธณเธฅเธฑเธเธฅเธ..." : `เธฅเธเธฃเธฒเธขเธเธฒเธฃเธ—เธตเนเน€เธฅเธทเธญเธ (${selectedIds.length})`}
                                    </button>
                                </div>
                            </div>
                        </div>
